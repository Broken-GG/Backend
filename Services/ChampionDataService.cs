...
namespace Backend.Services
{
    public class ChampionDataService : IChampionDataService
    {
        ...
        public ChampionDataService(HttpClient httpClient, ILogger<ChampionDataService> logger)
        {
        ...
        private async Task<string> GetLatestVersionAsync()
        {
            ...
            try
            {
                ...
                if (versions != null && versions.Length > 0)
                {
                    ...
                    _logger.LogInformation("✅ Latest Data Dragon version: {Version}", _latestVersion);
                    return _latestVersion;
                }
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "⚠️ Failed to fetch latest version, using fallback {FallbackVersion}", FallbackVersion);
            }
        ...
        private Task<Dictionary<long, string>> GetChampionMappingAsync()
        {
            ...
            lock (_lock)
            {
                ...
                try
                {
                    ...
                    _logger.LogInformation("✅ Loaded {Count} champions from Data Dragon", _championIdToKey.Count);
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "❌ Error loading champion data");
                    _championIdToKey = new Dictionary<long, string>();
                }
